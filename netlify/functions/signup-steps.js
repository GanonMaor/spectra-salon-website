...
async function ensureColumns(client, tableName, data) {
	for (const key of Object.keys(data)) {
		...
		if (col.rowCount === 0) {
			...
			await client.query(`ALTER TABLE ${tableName} ADD COLUMN ${key} ${sqlType}`);
		}
	}
}

function buildUpsertQuery(tableName, uniqueKey, data) {
	const filtered = Object.fromEntries(Object.entries(data).filter(([k]) => k !== 'updated_at'));
	const keys = Object.keys(filtered);
	const placeholders = keys.map((_, i) => `$${i + 1}`);
	const updates = keys.filter(k => k !== uniqueKey).map(k => `${k} = EXCLUDED.${k}`);
...
async function chooseTargetTable(client, incomingData) {
...
exports.handler = async (event) => {
	...
	try {
		...
		// Pick target table
		const target = await chooseTargetTable(client, body);
		if (target === 'users') {
		...
		return { statusCode: 200, headers, body: JSON.stringify({ success: true, user_id: res.rows[0].id, table: target }) };
	} catch (err) {
