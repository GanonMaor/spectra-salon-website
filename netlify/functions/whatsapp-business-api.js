    ...
    switch (httpMethod) {
      case "POST":
        if (path.includes("/send-template")) {
          ...
          // Media sending not implemented in this function version
          return {
            statusCode: 501,
            headers,
            body: JSON.stringify({ error: "send-media not implemented" }),
          };
        } else {
      ...
      case "GET":
        if (path.includes("/templates")) {
          ...
          // Basic status stub to avoid undefined reference
          return {
            statusCode: 200,
            headers,
...
async function logWhatsAppActivity(action, details) {
  ...
  try {
    ...
    await client.query(
      ...
      VALUES ($1, $2, $3, NOW())
    `,
      [`whatsapp_${action}`, "WhatsApp Business API", JSON.stringify(details)],
    );
  } catch (error) {
