...
const SUMMIT_API_URL = process.env.SUMMIT_API_URL;
...
// Fetch customers from SUMMIT API
async function fetchCustomers() {
  try {
    const response = await axios.get(`${SUMMIT_API_URL}/customers`, {
      headers: {
        'Authorization': `Bearer ${SUMMIT_API_KEY}`,
...
// Fetch payments from SUMMIT API
async function fetchPayments() {
  try {
    const response = await axios.get(`${SUMMIT_API_URL}/payments`, {
      headers: {
        'Authorization': `Bearer ${SUMMIT_API_KEY}`,
        'Content-Type': 'application/json'
      },
  ...
  for (const payment of payments) {
    try {
      ...
      const values = [
        ...
        payment.status,
        payment.service || 'Summit Service',
...
async function performDataSync() {
  ...
  try {
    console.log('Starting SUMMIT data sync...');
    
    // Validate environment variables
    if (!SUMMIT_API_URL || !SUMMIT_API_KEY || !NEON_DATABASE_URL) {
    ...
    // Fetch data from SUMMIT API
    console.log('Fetching customers from SUMMIT API...');
    const customers = await fetchCustomers();
    console.log(`Fetched ${customers.length} customers`);

    console.log('Fetching payments from SUMMIT API...');
