...
async function createMessage(client, event, headers) {
  const { ticket_id, sender_type, sender_name, sender_id, message, file_url } = JSON.parse(event.body);
  ...
  if (!['client', 'admin'].includes(sender_type)) {
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({ error: 'sender_type must be either "client" or "admin"' })
    };
  }

  // Check if ticket exists
  const ticketCheck = await client.query('SELECT id FROM support_tickets WHERE id = $1', [ticket_id]);
  ...
  await client.query(`
    ...
    WHERE id = $2
  `, [message.substring(0, 255), ticket_id]); // Limit to 255 chars for last_message
