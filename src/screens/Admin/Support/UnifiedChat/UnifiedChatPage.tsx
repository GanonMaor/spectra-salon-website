...
export const UnifiedChatPage: React.FC = () => {
  ...
  const fetchClientMessages = async (clientId: string) => {
    try {
      const response = await fetch(`/.netlify/functions/unified-messages?client_id=${clientId}&limit=50`);
  ...
  const handleUpdateMessage = async (messageId: string, updates: { status?: string; tag?: string; assigned_to?: string }) => {
    try {
      const response = await fetch(`/.netlify/functions/unified-messages/${messageId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        ...
        {selectedClient ? (
          ...
          <div className="flex items-center justify-center h-full text-gray-500">
            <div className="text-center">
              ...
              <p className="mt-1">Choose a client from the left to start chatting</p>
            </div>
          </div>
        )}
      </div>
