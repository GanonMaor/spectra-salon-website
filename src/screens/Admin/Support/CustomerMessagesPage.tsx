...
const CustomerMessagesPage: React.FC = () => {
  ...
  // Log view once per filter change (avoid depending on function identity)
  useEffect(() => {
    logDataView("customer_messages", {
      status_filter: statusFilter,
      stage_filter: stageFilter,
    });
    // intentionally omit logDataView from deps to prevent re-log loops
  ...
  const loadTickets = async () => {
    try {
      ...
      const response = await fetch(
        `/.netlify/functions/support-tickets?${params}`,
      );
      if (response.ok) {
  ...
  const loadMessages = async (ticketId: string) => {
    try {
      ...
      const response = await fetch(
        `/.netlify/functions/support-messages?ticket_id=${ticketId}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleTicketSelect = (ticket: SupportTicket) => {
    ...
    logButtonClick("ticket_selected", {
      ticket_id: ticket.id,
    ...
    try {
      ...
      if (viaWhatsApp && selectedTicket.phone) {
        ...
        if (template) {
          ...
          whatsappResponse = await fetch(
            "/.netlify/functions/whatsapp-business-api/send-template",
            {
              method: "POST",
          ...
          whatsappResponse = await fetch(
            "/.netlify/functions/whatsapp-business-api",
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
        ...
        if (!whatsappResponse.ok && whatsappResult.whatsappUrl) {
          ...
          throw new Error(
            whatsappResult.error || "Failed to send WhatsApp message",
          );
        }
      }
      ...
      const response = await fetch("/.netlify/functions/support-messages", {
        ...
        body: JSON.stringify({
          ...
          message: template
            ? `[Template: ${template.name}] ${replyMessage.trim()}`
            : replyMessage.trim(),
        }),
      });
  ...
  const updateTicketStatus = async (ticketId: string, status: string) => {
    try {
      ...
      if (response.ok) {
        ...
        logButtonClick("ticket_status_updated", {
          ticket_id: ticketId,
          new_status: status,
        });
      }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Manage support tickets and chat conversations
          </p>
        </div>
        {newTicketsCount > 0 && (
                ...
                filteredTickets.map((ticket) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      ...
                      <span
                        className={`px-2 py-1 rounded-full ${getStageColor(ticket.pipeline_stage)}`}
                      >
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
                        ...
                        {selectedTicket.source_page && (
                          <span className="text-blue-600">
                            From: {selectedTicket.source_page}
                          </span>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <button
                            onClick={() => {
                              setReplyMessage(
                                "Hi! Thanks for reaching out to Spectra. I'll be happy to help you with your color intelligence needs. When would be a good time for a quick call?",
                              );
                            }}
                          ...
                          <button
                            onClick={() => {
                              setReplyMessage(
                                "Great question! Let me send you our demo video and pricing information. You can also book a live demo at your convenience: [DEMO_LINK]",
                              );
                            }}
                          ...
                          <button
                            onClick={() => {
                              setReplyMessage(
                                "Thanks for your interest! Our pricing starts at $299/month for single salon with all features included. Would you like me to prepare a custom quote for your needs?",
                              );
                            }}
                          ...
                          <button
                            onClick={() => {
                              setReplyMessage(
                                "I understand you're having technical issues. Let me connect you with our technical support team right away. Can you describe the specific problem you're experiencing?",
                              );
                            }}
