...
const SignUpPage: React.FC = () => {
  ...
  // Initialize SUMIT Payments API when on payment step
  useEffect(() => {
    if (step !== 2 || !isTrial) return; // Only on payment step for trial
    
    // Wait for SUMIT script to load
    const initSumit = () => {
      if (typeof (window as any).OfficeGuy === 'undefined') {
        setTimeout(initSumit, 100);
        return;
      }
      ...
      try {
        ...
        console.warn('SUMIT Payments API initialization failed:', error);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isTrial) {
      try {
        ...
        // Get SUMIT token from form (generated by OfficeGuy.Payments)
        const formElement = e.target as HTMLFormElement;
        const ogTokenInput = formElement.querySelector('input[name="og-token"]') as HTMLInputElement;
        ...
        const paymentRes = await fetch("/.netlify/functions/sumit-payment", {
          ...
          body: JSON.stringify({
            ...
            singleUseToken: ogToken, // SUMIT secure token instead of card details
            metadata: {
