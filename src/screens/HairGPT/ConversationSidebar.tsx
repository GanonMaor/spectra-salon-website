      ...
      >
        ...
        <div className="p-4 pb-3">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              <SpectraLogo size={28} />
              <span className="text-[13px] font-light text-white/40 tracking-wide">HairGPT</span>
            </div>
            <button
              onClick={onToggle}
          ...
          <button
            onClick={onNew}
            className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl border border-white/[0.06] hover:border-[#EAB776]/15 bg-white/[0.02] hover:bg-[#EAB776]/[0.05] text-white/35 hover:text-white/60 text-[13px] font-light transition-all duration-300"
          >
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-0.5">
          {conversations.length === 0 && (
            <p className="text-white/12 text-[11px] text-center py-12 px-4 font-light">
              {t.noChats}
            </p>
          )}
          {conversations.map((c) => (
