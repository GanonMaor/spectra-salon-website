 ...
 * Booking flow modal.
 *
 * A staged, composition-based appointment builder rendered over the schedule
 * page. The user picks an entry type, a client, builds a multi-service
 * composition, edits the generated workflow, checks scheduling/conflicts,
 * and reviews before creating. A persistent summary panel stays visible.
 *
 * Reuses the existing CRM glass/Tailwind visual language. Non-appointment
 * entry types use a compact dedicated form that still persists a calendar
 * block, so every visible option works.
 */

import React, { useMemo, useState, useCallback } from "react";
import { X, Search, Check, AlertCircle, ChevronRight } from "lucide-react";
...
interface Props {
  ...
  onClose: () => void;
  onSubmit: (payload: CompositionCreatePayload) => void;
}
...
export const BookingFlowModal: React.FC<Props> = ({
  ...
  const goToStep = (next: FlowStep) => {
    const idx = FLOW_STEPS.indexOf(next);
    setStep(next);
  ...
  const handleNext = () => {
    ...
    const nextIdx = Math.min(stepIndex + 1, FLOW_STEPS.length - 1);
    goToStep(FLOW_STEPS[nextIdx]);
  };
  ...
  const handleCreate = () => {
    ...
    onSubmit(buildCreatePayload(composition));
    onClose();
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4" onClick={onClose}>
      ...
      >
        ...
        <div className={`flex items-center justify-between px-6 py-4 border-b ${isDark ? "border-white/[0.08]" : "border-black/[0.06]"}`}>
          <div>
            <p className={`text-base font-bold ${textStrong}`}>New Calendar Entry</p>
        ...
        <div className="flex flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto scrollbar-thin p-6">
            ...
            {step === "review" && (
              <div className="space-y-4">
                <p className={`text-[13px] font-semibold ${textStrong}`}>Review & Create</p>
        ...
        {/* Footer */}
        {isAppointment && (
          <div className={`flex items-center justify-between px-6 py-3 border-t ${isDark ? "border-white/[0.08]" : "border-black/[0.06]"}`}>
            ...
            <button
              ...
              style={{ background: "linear-gradient(315deg, #9a7544, #c79c6d)" }}
            >
              {step === "review" ? "Create Appointment" : "Continue"}
            </button>
          </div>
        )}
      </div>
