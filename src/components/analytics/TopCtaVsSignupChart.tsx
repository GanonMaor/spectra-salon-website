...
export default function TopCtaVsSignupChart({ limit = 5, className }: Props) {
  ...
  const data = React.useMemo(() => {
    const items = (rows || []).map((r) => {
      const cta = r.cta_path || "—";
      const sign = r.signup_path || "—";
      const leads = typeof r.leads === "string" ? parseInt(r.leads, 10) : (r.leads || 0);
      return { label: `${cta} → ${sign}`, leads };
    });
    ...
    <div className={`h-full ${className || ""}`}>
      <div className="mb-4">
        <div className="flex items-center gap-2">
          <h3 className="text-white font-medium text-sm">Top CTAs ↔ Signup (30d)</h3>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 8, right: 16, left: 8, bottom: 40 }}>
                ...
                <Bar dataKey="leads" radius={[6, 6, 0, 0]} fill="#f97316" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
