...
async function checkCustomerData() {
  ...
  try {
    ...
    const sumitCount = await client.query('SELECT COUNT(*) FROM sumit_customers');
    console.log(`📊 סה"כ לקוחות ב-sumit_customers: ${sumitCount.rows[0].count}`);
    ...
    `);
    console.log(`📊 לקוחות ב-summit_detailed_payments (עם ID): ${summitWithId.rows[0].count}`);
    ...
    const summitTotal = await client.query(`
      ...
      FROM summit_detailed_payments
    `);
    console.log(`📊 סה"כ שמות לקוחות ב-summit_detailed_payments: ${summitTotal.rows[0].count}`);
    ...
    const churnCount = await client.query('SELECT COUNT(*) FROM churn_analysis');
