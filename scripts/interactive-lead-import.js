...
async function interactiveImport() {
  ...
  while (true) {
    ...
    const sourceChoice = await question(
      "Select source page (1-6, default: 6): ",
    );
    const sourcePages = [
      "/",
      "/special-offer",
    ...
    const source_page =
      sourcePages[parseInt(sourceChoice) - 1] || "/crm-import";

    const utm_source =
      (await question("UTM Source (optional, default: crm-import): ")) ||
      "crm-import";
    const utm_medium =
      (await question("UTM Medium (optional, default: import): ")) || "import";
    const utm_campaign =
      (await question("UTM Campaign (optional, default: legacy-crm): ")) ||
      "legacy-crm";
  ...
  leads.forEach((lead, index) => {
    console.log(
      `${index + 1}. ${lead.full_name} - ${lead.email} (${lead.source_page})`,
    );
  });
