...
(async () => {
  try {
    ...
    console.log(
      `✅ Merge complete! ${merged.length} unique customers saved to all_sumit_customers.csv`,
    );
    if (duplicates.length) {
      console.log(
        `⚠️  ${duplicates.length} duplicates saved to duplicates.csv`,
      );
    } else {
