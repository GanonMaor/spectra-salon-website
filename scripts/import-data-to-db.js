...
async function importPayments(client) {
  ...
  const paymentsFile = path.join(__dirname, 'data/normalized/sumit_payments.csv');
  const payments = [];
    ...
    fs.createReadStream(paymentsFile)
      ...
      .on('data', (row) => {
        payments.push({
          ...
          customer_name: cleanString(row['customer_name']) || cleanString(row['שם הכרטיס']),
          amount: parseAmount(row['amount']),
          product: cleanString(row['service_name']),
          method: cleanString(row['סוג תשלום']),
          status: cleanString(row['status']) || 'completed',
          payment_date: parseDate(row['payment_date']) || parseDate(row['created_at'])
        });
      })
      .on('end', async () => {
        try {
...
async function main() {
  ...
  try {
    ...
    // Verify data
    const customerCount = await client.query('SELECT COUNT(*) FROM sumit_customers');
    const paymentCount = await client.query('SELECT COUNT(*) FROM sumit_payments');
    const totalAmount = await client.query('SELECT SUM(amount) FROM sumit_payments');
    
    console.log('\n📊 Database Summary:');
    console.log(`👥 Total customers: ${customerCount.rows[0].count}`);
    console.log(`💰 Total payments: ${paymentCount.rows[0].count}`);
    console.log(`💵 Total amount: ₪${parseFloat(totalAmount.rows[0].sum || 0).toLocaleString()}`);
