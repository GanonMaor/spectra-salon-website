...
// קובץ המקור
const INPUT_FILE = path.join(__dirname, 'data', 'sumit_all_paymants_history.xlsx');
// קובץ ה-CSV המתוקן
...
function normalizeRow(row) {
  ...
  // המרת סכום למספר
  if (out.amount) {
    out.amount = parseFloat(String(out.amount).replace(/[^\d.]/g, '')).toFixed(2);
  }
...
function main() {
  ...
  csvWriter.writeRecords(normalizedRows)
    .then(() => {
      console.log(`✅ Normalized payments saved to ${OUTPUT_FILE}`);
      console.log(`שמור קובץ זה ותוכל לייבא אותו ל-Neon עם \copy או ממשק גרפי.`);
    })
    .catch(err => {
