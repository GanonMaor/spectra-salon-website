...
// Files
const PAYMENTS_FILE = path.join(__dirname, 'data', 'raw', 'summit_payments_detailed.xlsx');
...
async function importPayments(client) {
  ...
  for (const row of rawData) {
    try {
      ...
      const paymentData = {
        payment_id: cleanString(row['מזהה'] || row['ID'] || row['Payment ID'] || ''),
        customer_name: cleanString(row['שם לקוח'] || row['Customer Name'] || row['לקוח'] || ''),
        ...
        service_type: cleanString(row['סוג שירות'] || row['Service'] || row['Product'] || ''),
...
async function importCustomers(client) {
  ...
  for (const row of rawData) {
    try {
      ...
      const customerData = {
        customer_id: cleanString(row['מזהה לקוח'] || row['Customer ID'] || row['ID'] || ''),
        customer_name: cleanString(row['שם'] || row['Name'] || row['Customer Name'] || ''),
        email: cleanString(row['מייל'] || row['Email'] || ''),
        phone: cleanString(row['טלפון'] || row['Phone'] || ''),
        created_date: formatDate(excelDateToJS(row['תאריך הצטרפות'] || row['Created Date'] || row['Signup Date'])),
