...
function normalizeCompletedPayments() {
  ...
  fs.createReadStream(input)
    .pipe(csv())
    .on('data', (row) => {
      const { id, card_name, reference_number, payment_date, customer_name, amount, product_or_service, payment_method, status, linked_documents, notes, created_at, country } = row;
...
function normalizeFailedPayments() {
  ...
  const results = rows.map((row) => {
    ...
    const attempted_at = row['תאריך'] ? dayjs(row['תאריך']).format('YYYY-MM-DD HH:mm:ss') : '';
