...
function normalizeStandingOrders() {
  ...
  const results = rows.map((row) => {
    const {
      ...
      סטטוס: status,
      "כמות יחידות": quantity,
      'מחיר כולל מע"מ': price_with_vat,
      'סה"כ כולל מע"מ': total_with_vat,
      "הפקת מסמך בלבד": document_only,
      "מחזורים שנותרו": remaining_cycles,
      "אמצעי תשלום פעיל": payment_method_raw,
    } = row;

    const { id: customer_id, name: customer_name } = splitIdName(
      customer_raw || "",
    );
    const { method_id: payment_method_id, last_digits: card_last_digits } =
      splitPaymentMethod(payment_method_raw || "");

    return {
      id: id || "",
      next_charge_date: next_charge_date
        ? dayjs(next_charge_date).format("YYYY-MM-DD")
        : "",
      customer_id,
      customer_name,
