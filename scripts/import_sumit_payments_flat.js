...
const FILE = path.join(__dirname, 'data/normalized/sumit_monthly_payments_by_customer.xlsx');
...
async function main() {
  ...
  const monthCols = header.filter(h => h && h.match(/^\d{2}\/\d{4}|\d{4}-\d{2}$/));

  let inserted = 0;
  for (const row of rows) {
    const customer_id = row['מזהה לקוח/ה'] || row['ת"ז'] || row['ת.ז'] || row['id'] || row['customer_id'];
    const full_name = row['לקוח/ה'] || row['שם הכרטיס'] || row['שם מלא'] || row['full_name'];
    ...
    for (const monthCol of monthCols) {
      ...
      if (amount !== 0) {
        ...
        await client.query(
          ...
          [customer_id, full_name, email, phone, city, address, status, month, amount]
        );
        inserted++;
      }
    }
  ...
  await client.end();
  console.log(`✅ Imported ${inserted} flat payment rows to sumit_payments_flat`);
}
