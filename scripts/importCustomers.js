    ...
    fs.createReadStream('scripts/data/sumit_customers_new.csv')
      ...
      .on('data', (row) => {
        ...
        if (errorReason) {
          ...
          skipCount++;
          console.warn(`⚠️ Skipping invalid row: ${customer.full_name || 'Unknown'} - ${errorReason}`);
          return;
        }
      ...
      .on('end', async () => {
        ...
        if (failedRecords.length > 0) {
          try {
            ...
            console.log(`💾 Saved ${failedRecords.length} failed records to import_errors_customers.csv`);
          } catch (err) {
        ...
        for (const customer of customers) {
          try {
            ...
            successCount++;
            console.log(`✅ Processed: ${customer.full_name} (${customer.email})`);
          } catch (err) {
            errorCount++;
            customer.error_reason = err.message;
            failedRecords.push(customer);
            console.error(`❌ Error inserting ${customer.full_name}:`, err.message);
          }
        }
