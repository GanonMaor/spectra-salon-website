...
const importCustomers = async () => {
  ...
  if (!fs.existsSync('scripts/data/sumit_customers_new.csv')) {
    console.error('❌ Customer CSV file not found: scripts/data/sumit_customers_new.csv');
    ...
    fs.createReadStream('scripts/data/sumit_customers_new.csv')
      ...
      .on('data', (row) => {
        ...
        if (errorReason) {
          ...
          skipCount++;
          console.warn(`⚠️ Skipping invalid customer: ${customer.full_name || 'Unknown'} - ${errorReason}`);
          return;
        }
      ...
      .on('end', async () => {
        console.log(`📥 Total customer records read: ${customers.length + skipCount}`);
        ...
        if (failedRecords.length > 0) {
          try {
            ...
            console.log(`💾 Saved ${failedRecords.length} failed customer records to import_errors_customers.csv`);
          } catch (err) {
            console.error('❌ Error saving failed customer records:', err.message);
          }
        }
            ...
            await pool.query(
              `INSERT INTO users 
               ...
               ON CONFLICT (email) DO UPDATE SET
                 ...
                 updated_at = NOW()`,
              [
                customer.card_name, customer.full_name, customer.id_number,
                customer.phone, customer.email, customer.address,
                customer.city, customer.zip_code, customer.next_contact, customer.status
              ]
            );
            successCount++;
            console.log(`✅ Processed customer: ${customer.full_name} (${customer.email})`);
          } catch (err) {
            errorCount++;
            customer.error_reason = err.message;
            failedRecords.push(customer);
            console.error(`❌ Error inserting customer ${customer.full_name}:`, err.message);
          }
        }
...
const runAllImports = async () => {
  ...
  try {
    ...
    if (fs.existsSync('scripts/data/sumit_payments.csv')) {
      ...
      results.payments = { success: false, message: 'Payment CSV file not found' };
    }
    ...
    if (results.customers?.success) {
      console.log(`👥 Customers: ${results.customers.summary.imported} imported, ${results.customers.summary.skipped} skipped, ${results.customers.summary.failed} failed`);
    }
    
    if (results.payments?.success) {
      console.log(`💰 Payments: ${results.payments.summary.imported} imported, ${results.payments.summary.skipped} skipped, ${results.payments.summary.failed} failed`);
    }
