      ...
      .on('end', async () => {
        try {
          ...
          for (const customer of customers) {
            if (!customer.full_name && !customer.email && !customer.customer_id) continue;
            ...
            try {
              ...
              console.error('❌ Error importing customer:', customer.full_name, err.message);
            }
          }
...
async function importEnhancedPayments(client) {
  ...
  const paymentsFile = path.join(__dirname, 'data/normalized/sumit_payments_with_id.csv');
  const payments = [];
        ...
        try {
          ...
          for (const payment of payments) {
            ...
            try {
              ...
              console.error('❌ Error importing payment:', payment.payment_id, err.message);
            }
          }
          
          console.log(`✅ Imported ${imported} payments`);
          console.log(`📅 Valid dates: ${validDates}/${imported}`);
          console.log(`📊 Monthly distribution:`, Object.keys(dateStats).sort());
...
async function generateAnalytics(client) {
  ...
  try {
    ...
    monthlyAnalysis.rows.forEach(row => {
      console.log(`${row.month.toISOString().substring(0, 7)}: ${row.payment_count} payments, ${row.unique_customers} customers, ₪${parseFloat(row.total_revenue).toLocaleString()}`);
    });
    ...
    customerAnalysis.rows.forEach((row, i) => {
      console.log(`${i+1}. ${row.customer_name}: ₪${parseFloat(row.lifetime_value).toLocaleString()} (${row.total_payments} payments, ${Math.round(row.active_months)} months)`);
    });
    ...
    console.log('\n🎯 Business Overview:');
    console.log(`📅 Active period: ${stats.business_start?.toISOString().substring(0, 10)} to ${stats.last_activity?.toISOString().substring(0, 10)}`);
    ...
    console.log(`📊 Average payment: ₪${parseFloat(stats.avg_payment).toFixed(2)}`);
